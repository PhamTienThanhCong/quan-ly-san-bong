...
const UpdateProfileComponent = ({ currentUser, updateUser }) => {
  const [formData, setFormData] = useState({
    ...
    avatar: "",
    bio: currentUser.role === ROLE_MANAGER.SALE ? currentUser.bio || "" : "",
    bank_info: currentUser.role === ROLE_MANAGER.SALE ? currentUser.bank_info || "" : "",
    bank_info_number: currentUser.role === ROLE_MANAGER.SALE ? currentUser.bank_info_number || "" : ""
  });
  ...
  const validateForm = () => {
    ...
    if (currentUser.role === ROLE_MANAGER.SALE) {
      ...
      {currentUser.role === ROLE_MANAGER.SALE && (
        <>
          ...
          <Form.Group className="mb-3">
            ...
            <Form.Control
              type="text"
              name="bank_info_number"
              value={formData.bank_info_number}
              onChange={handleChange}
            />
            {errors.bank_info_number && <div className="text-danger">{errors.bank_info_number}</div>}
          </Form.Group>

          <p className="text-danger">Vui lòng cập nhập số đúng số tài khoản ngân hàng của bạn.</p>
        </>
      )}
