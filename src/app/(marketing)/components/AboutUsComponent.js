import { PHONE_NUMBER } from "@quanlysanbong/constants/MainContent";
import Link from "next/link";

const AboutUsComponent = () => {
  return (
    <div className="container-fluid about py-5">
      <div className="container py-5">
        <div className="row g-5 align-items-center">
          <div className="col-xl-7 wow fadeInLeft" data-wow-delay="0.2s">
            <div>
              <h4 className="text-primary">Về Chúng Tôi</h4>
              <h1 className="display-5 mb-4">Đặt sân dễ dàng - Trải nghiệm tuyệt vời</h1>
              <p className="mb-4">
                Chúng tôi cung cấp hệ thống đặt sân đá trực tuyến nhanh chóng, giúp bạn dễ dàng tìm kiếm và thuê
                sân với chất lượng tốt nhất.
              </p>
              <div className="row g-4">
                <div className="col-md-6 col-lg-6 col-xl-6">
                  <div className="d-flex">
                    <div>
                      <i className="fas fa-futbol fa-3x text-primary"></i>
                    </div>
                    <div className="ms-4">
                      <h4>Hệ Thống Tiện Lợi</h4>
                      <p>Đặt sân nhanh chóng, dễ dàng với hệ thống trực tuyến 24/7.</p>
                    </div>
                  </div>
                </div>
                <div className="col-md-6 col-lg-6 col-xl-6">
                  <div className="d-flex">
                    <div>
                      <i className="bi bi-calendar-check-fill fa-3x text-primary"></i>
                    </div>
                    <div className="ms-4">
                      <h4>Nhiều Năm Kinh Nghiệm</h4>
                      <p>Chúng tôi có kinh nghiệm trong việc quản lý và cho thuê sân.</p>
                    </div>
                  </div>
                </div>
                <div className="col-sm-6">
                  <Link href="/san-bong" className="btn btn-primary rounded-pill py-3 px-5 flex-shrink-0">
                    Đặt Sân Ngay
                  </Link>
                </div>
                <div className="col-sm-6">
                  <div className="d-flex">
                    <i className="fas fa-phone-alt fa-2x text-primary me-4"></i>
                    <div>
                      <h4>Liên Hệ Chúng Tôi</h4>
                      <p className="mb-0 fs-5" style={{ letterSpacing: "1px" }}>
                        {PHONE_NUMBER}
                      </p>
                    </div>
                  </div>
                </div>
              </div>
            </div>
          </div>
          <div className="col-xl-5 wow fadeInRight" data-wow-delay="0.2s">
            <div className="bg-primary rounded position-relative overflow-hidden">
              <img src="img/about-2.png" className="img-fluid rounded w-100" alt="" />

              <div style={{ position: "absolute", top: "-15px", right: "-15px" }}>
                <img
                  src="img/about-3.png"
                  className="img-fluid"
                  style={{ width: "150px", height: "150px", opacity: 0.7 }}
                  alt="About Image 3"
                />
              </div>
              <div style={{ position: "absolute", top: "-20px", left: "10px", transform: "rotate(90deg)" }}>
                <img
                  src="img/about-4.png"
                  className="img-fluid"
                  style={{ width: "100px", height: "150px", opacity: 0.9 }}
                  alt="About Image 4"
                />
              </div>
              <div className="rounded-bottom">
                <img src="img/about-5.jpg" className="img-fluid rounded-bottom w-100" alt="About Image 5" />
              </div>
            </div>
          </div>
        </div>
      </div>
    </div>
  );
};

export default AboutUsComponent;
